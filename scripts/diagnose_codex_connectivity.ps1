...
Write-Host "== Codex / AI CLI Connectivity Diagnostic: Windows PowerShell =="
Write-Host "This script prints proxy, loopback, DNS, target-domain, and optional callback-port diagnostics."
...
if ($Port -ne "") {
    ...
    Write-Host ".\diagnose_codex_connectivity.ps1 -Port 1455 -TargetDomain api.openai.com"
    Write-Host ""
}
...
Write-Host "== HTTPS reachability check without explicit proxy =="
try {
