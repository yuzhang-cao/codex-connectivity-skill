...
echo "== Codex / AI CLI Proxy Assist: macOS/Linux =="
echo "This script discovers system-visible local proxy candidates and prints temporary retry commands."
...
if [[ -n "$PORT" ]]; then
  echo "== Optional callback port support: $PORT =="
  if command -v lsof >/dev/null 2>&1; then
  ...
  echo "== Optional callback port support skipped =="
  echo "Pass callback port as first arg and target domain as second arg, for example:"
  echo "./proxy_assist.sh 1455 api.openai.com"
  echo
fi

echo "== Target-domain fallback check =="
